package pink.zak.giveawaybot.commands.discord.preset.subs.exports.utils;
...
public class ImportCmdUtils extends ListenerAdapter {
    ...
    public ImmutablePair<Set<String>, Map<String, Preset>> getAffectedPresets(Server server, Message message, JsonObject jsonObject) {
        ...
        Map<String, Preset> cachedValues = this.serializedCache.getSync(messageId);
    ...
    public void onGuildMessageReactionAdd(@NotNull GuildMessageReactionAddEvent event) {
        ...
        this.serverCache.get(event.getGuild().getIdLong()).thenAccept(server -> {
            ...
            Map<String, Preset> toAdd = this.serializedCache.getSync(messageId);
            if (toAdd != null) {
            ...
            if (json != null) {
                ...
                try {
                    ...
                    if (name == null || name.length() < 3 || presetValues == null || presetValues.isEmpty()) {
                        ...
                        this.languageRegistry.get(server, Text.PRESET_IMPORTED_SINGULAR, replacer -> replacer.set("preset", preset.name())).to(channel);
                    }
                } catch (Exception ex) {
