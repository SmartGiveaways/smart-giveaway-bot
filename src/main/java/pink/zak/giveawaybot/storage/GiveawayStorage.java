...
public class GiveawayStorage extends Storage<Giveaway> implements CacheLoader<UUID, Giveaway>, CacheSaver<UUID, Giveaway> {
    ...
    public Serializer<Giveaway> serializer() {
        return (giveaway, json, gson) -> {
            json.addProperty("_id", giveaway.uuid().toString());
            json.addProperty("messageId", String.valueOf(giveaway.messageId()));
    ...
    public Deserializer<Giveaway> deserializer() {
        return (json, gson) -> {
            ...
            return new Giveaway(uuid, messageId, channelId, serverId, startTime, endTime, winnerAmount, presetName, giveawayItem, enteredUsers);
        };
    }
