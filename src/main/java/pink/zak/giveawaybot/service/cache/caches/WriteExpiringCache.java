...
public class WriteExpiringCache<K, V> extends Cache<K, V> {
    ...
    public WriteExpiringCache(GiveawayBot bot, Storage<V> storage, CacheExpiryListener<K, V> expiryListener, Consumer<V> removalAction, TimeUnit timeUnit, int delay, TimeUnit autoSaveUnit, int autoSaveInterval) {
    ...
    public WriteExpiringCache(GiveawayBot bot, Storage<V> storage, CacheExpiryListener<K, V> expiryListener, Consumer<V> removalAction, TimeUnit timeUnit, int delay) {
        this(bot, storage, expiryListener, removalAction, timeUnit, delay, null, 0);
    }

    public WriteExpiringCache(GiveawayBot bot, Storage<V> storage, TimeUnit timeUnit, int delay, TimeUnit autoSaveUnit, int autoSaveInterval) {
        this(bot, storage, null, null, timeUnit, delay, autoSaveUnit, autoSaveInterval);
    }

    public WriteExpiringCache(GiveawayBot bot, Storage<V> storage, TimeUnit timeUnit, int delay) {
